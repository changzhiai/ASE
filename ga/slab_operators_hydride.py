...
def permute2(atoms, rng=np.random, element_pools=None):
    if bool(element_pools):
        ...
        list1 = [a.index for a in atoms if a.symbol == element_pools[0]]
        list2 = [a.index for a in atoms if a.symbol == element_pools[1]]
        if bool(list1) and bool(list2):
            i1 = rng.choice([a.index for a in atoms if a.symbol == element_pools[0]])
            sym1 = atoms[i1].symbol
            i2 = rng.choice([a.index for a in atoms if a.symbol == element_pools[1]])
            atoms[i1].symbol = atoms[i2].symbol
            atoms[i2].symbol = sym1
        else:
...
class SymmetrySlabPermutation(SlabOperator):
    ...
    def get_new_individual(self, parents):
        ...
        if bool(self.element_pools):
            list1 = [a.index for a in f if a.symbol == self.element_pools[0]]
            list2 = [a.index for a in f if a.symbol == self.element_pools[1]]
